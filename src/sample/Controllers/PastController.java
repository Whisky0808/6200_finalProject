...
public class PastController  implements Initializable {
    ...
    private void LoadTodayItems(){
        FilterFuture=(TodoItem)-> TodoItem.getDeadline().isBefore(LocalDate.now());
        PastItems=new FilteredList<TodoItem>(TodoData.getInstance().getTodoItems(), FilterFuture);
    }
    private void LoadShowItems(){
        ShowItems = new FilteredList<TodoItem>(PastItems, s -> {
            int index = PastItems.indexOf(s);
